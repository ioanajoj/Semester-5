...
Matrix operator+(Matrix const & matrix1, Matrix const & matrix2)
{
	...
	for (int i = 0; i < cells; ++i) {
		futures.push_back(std::async(add_matrix, sum_matrix, i, rows, columns, matrix1, matrix2));
	}
...
Matrix operator*(Matrix const & matrix1, Matrix const & matrix2)
{
	...
	std::vector<std::future<void>> futures;
	for (int i = 0; i < cells; ++i) {
		futures.push_back(std::async(multiply_lc, product_matrix, i, matrix1, matrix2));
	}	
