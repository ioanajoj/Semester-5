...
int main(int argc, char* argv[])
{
	...
	if (pid == 0)
	{
		...
		if (BROADCAST)
		{
			...
			MPI_Bcast(P, size_P, MPI_LONG, 0, MPI_COMM_WORLD);
			MPI_Bcast(Q, size_Q, MPI_LONG, 0, MPI_COMM_WORLD);
		}
		else
		{
			for (int i = 1; i < np; i++)
	...
	else
	{
		...
		if (BROADCAST)
		{
			MPI_Bcast(P, metadata[0], MPI_LONG, 0, MPI_COMM_WORLD);
			MPI_Bcast(Q, metadata[1], MPI_LONG, 0, MPI_COMM_WORLD);
		}
		else
		{
		...
		// send back result
		MPI_Ssend(Res, size_C, MPI_LONG, 0, 3, MPI_COMM_WORLD);
	}
