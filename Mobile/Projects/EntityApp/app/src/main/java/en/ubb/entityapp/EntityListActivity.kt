...
import en.ubb.entityapp.adapter.MyAdapter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fabRefresh.setOnClickListener { v ->
            if (!manager.networkConnectivity(this)) {
                ...
                model.fetchEntitiesFromNetwork(application as EntityApp)
            }
        }
...
//        model.message.observe { showError(it) }
    ...
    private fun loadEntities() {
        model.fetchEntities(application as EntityApp)
    }

    private fun setupRecyclerView(recyclerView: RecyclerView) {
        adapter = MyAdapter()
        recyclerView.adapter = adapter
        (application as EntityApp).db.entityDao.entities
