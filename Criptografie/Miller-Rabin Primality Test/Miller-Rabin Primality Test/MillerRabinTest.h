...
private:
	...
	*/
	std::vector<int> get_multiplication_sequence(unsigned int a, unsigned int s, unsigned int t);
	...
	*/
	bool check_multiplication_sequence(std::vector<int> sequence);


	unsigned long int int_pow(int b, int e);

public:
	MillerRabinTest(unsigned int n, unsigned int k);
	~MillerRabinTest();
